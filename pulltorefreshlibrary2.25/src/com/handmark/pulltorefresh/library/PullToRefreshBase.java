...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public void firstRefreshing(){
		Log.e("TAG","firstRefreshing----在执行---");
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				while (true){
					if (mHeaderLayout.getWidth() > 0) {
						Log.e("TAG","firstRefreshing----在刷新---");
						setRefreshing(true);//刷新
						break;
					}
					SystemClock.sleep(100);//sleep一小段时间
				}
			}
		}, 500);
	}
	...
	private void complete(boolean noData) {
		if (isRefreshing()) {
...
//		getLoadingLayoutProxy().setLastUpdatedLabel(label);
		...
		if (null != mFooterLayout) {
			mFooterLayout.setSubHeaderText("");
		}
	}
    ...
    public int findIndexOfValue(String[] mEntryValues, String value) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String[] packages = context.getResources().getStringArray(R.array.package_entries);
		final int index = findIndexOfValue(packages, packageName);
		...
		if(index < 0) {
			...
			TypedArray styles = context.getResources().obtainTypedArray(R.array.package_values);
			final int sytleId = styles.getResourceId(index, 0);
			styles.recycle();
			styles = null;
			a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefresh, 0, sytleId);
		}
	 ...
	 * 
	 * @return true if net is wifi type, false if wifi is disabled.
	 */
	public boolean isWifiState() {
//		NetworkInfo networkInfo = mConnectivityManager.getActiveNetworkInfo();
//		if (networkInfo == null) {
//			Log.w(LOG_TAG, "no active network.");
//			return false;
//		}
//		int type = networkInfo.getType();
//
//		if (type == ConnectivityManager.TYPE_WIFI) {
//			Log.w(LOG_TAG, "is wifi network type.");
//			return true;
//		}
//		
//		Log.w(LOG_TAG, "is mobile network type.");
		return false;
	}
