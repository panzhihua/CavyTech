...
//        dbHelper.close();
    ...
    public void init(String sdCardFolderAddress, int maxChunks, DownloadManagerListener listener){
        ...
        tasksDataSource.initDowningData();
    }
    ...
    public void waitPauseDownload(int token){
        moderator.pauseToWait(token);
    }

    public void waitInitDownload(int token){
        moderator.initToWait(token);
    }
    public void changeStateToInit(int token){
        moderator.changeStateToInit(token);
    }
    public void changeStateToPause(int token){
        moderator.changeStateToPause(token);
    }
    public void changeStateToDownloading(int token){
        moderator.changeStateToDownloading(token);
    }
    /**
