...
public class WebActivity extends CommonActivity {
	...
	private void fillView() {
		...
		webUrl = getIntent().getStringExtra("webUrl")==null?"":getIntent().getStringExtra("webUrl");
		LogUtil.getLogger().d(webUrl);
		if (webUrl.length() > 0) {
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				...
				if (url.startsWith("tel:")) {
					...
					if (TelephonyManager.PHONE_TYPE_NONE == tm.getPhoneType()) {
						CustomToast.showToast(getCoreApplication(), "您使用的设备无电话功能");
					} else {
